import { Dialog, Transition } from "@headlessui/react";
import { Fragment } from "react";
import Form from "./FormComponent";
import { RequestForm as FormContent } from "../content/contents";
import { SubmitHandler, useForm } from "react-hook-form";
import { RequestData } from "../types/types";
import { toast, Toaster } from "react-hot-toast";
import { useRequestContext } from "../context/RequestContext";

interface Props {
	isOpen: boolean;
	closeModal: () => void;
}

const RequestForm = ({ closeModal, isOpen }: Props) => {
	const { addRequest } = useRequestContext();

	const { reset } = useForm();

	const onSubmit: SubmitHandler<RequestData> = async (formData) => {
		toast.promise(addRequest(formData), {
			loading: "Your request is being process",
			success: "Your request is created",
			error: "Something went wrong",
		});

		reset();
	};

	return (
		<>
			<Toaster />
			<Transition appear show={isOpen} as={Fragment}>
				<Dialog as="div" className="relative z-10" onClose={closeModal}>
					<Transition.Child
						as={Fragment}
						enter="ease-out duration-300"
						enterFrom="opacity-0"
						enterTo="opacity-100"
						leave="ease-in duration-200"
						leaveFrom="opacity-100"
						leaveTo="opacity-0"
					>
						<div className="fixed inset-0 bg-black bg-opacity-25" />
					</Transition.Child>

					<div className="fixed inset-0 overflow-y-auto">
						<div className="flex min-h-full items-center justify-center p-4 text-center">
							<Transition.Child
								as={Fragment}
								enter="ease-out duration-300"
								enterFrom="opacity-0 scale-95"
								enterTo="opacity-100 scale-100"
								leave="ease-in duration-200"
								leaveFrom="opacity-100 scale-100"
								leaveTo="opacity-0 scale-95"
							>
								<Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
									<Dialog.Title
										as="h3"
										className="text-center text-lg font-medium leading-6 text-gray-900"
									>
										Request For Player
									</Dialog.Title>
									<div className="mt-2">
										<Form
											formFields={FormContent}
											onSubmit={onSubmit}
											form={"Request"}
											buttonType={"submit"}
											buttonText={"create"}
											className="mb-5"
										/>
									</div>
								</Dialog.Panel>
							</Transition.Child>
						</div>
					</div>
				</Dialog>
			</Transition>
		</>
	);
};

export default RequestForm;
